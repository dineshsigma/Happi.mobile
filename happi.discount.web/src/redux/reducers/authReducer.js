...
export const OtpRequest = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    try {
      const response = await axios.post(
        `https://happi-ticket-api-qa-4at4ivbija-uc.a.run.app/api/employee/forgotPassword`,
        payload
      );
      // console.log("response-----", response);
      if (response.data.status) {
        data = {
          success: true,
          message: "OTP sent on your registered mobile over call",
        };
...
export const verifyOTP = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    try {
      const response = await axios.post(
        `https://happi-ticket-api-qa-4at4ivbija-uc.a.run.app/api/employee/verifiyOtp`,
        payload
      );
      // console.log("response-----", response);
      if (response.data.status) {
...
export const changePassword = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    try {
      const response = await axios.post(
        `https://happi-ticket-api-qa-4at4ivbija-uc.a.run.app/api/employee/resetPassword`,
        payload
      );
      // console.log("response-----", response);
      if (response.data.status) {
...
export const loginVerifyOTP = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    try {
      const response = await axios.post(
        `https://happi-ticket-api-qa-4at4ivbija-uc.a.run.app/api/employee/loginverifiyOtp`,
        payload
      );
      // console.log("response-----", response);
      if (response.data.status) {
...
export const adminlogin = createAsyncThunk('auth/adminlogin', async (payload, thunkAPI) => {
  ...
  let data = {}
  try {
    const response = await axios.post(`https://happi-ticket-api-qa-4at4ivbija-uc.a.run.app/api/employee/discountEmployeeLogin`, payload);
    // console.log("response-----", response);
    if (response.data.status) {
      data = {
        success: true,
  ...
  extraReducers: {
    ...
    [OtpRequest.fulfilled]: (state, action) => {
      if(action.payload.status){
        state.user_id = action.payload.employeeResponse._id;
      }
      return state;
    },
    [adminlogin.fulfilled]: (state, action) => {
      ...
      //   state.UserDetails = action.payload?.userId
      //   localStorage.setItem('token', action.payload.token)
      // }
      return state;
    },
    [loginVerifyOTP.fulfilled]: (state, action) => {
