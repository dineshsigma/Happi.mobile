...
import { updateDiscountToken, updatetoken } from "../reducers/auth";
...
const DiscountProfile = () => {
    ...
    // verify_data
    React.useEffect(()=>{
        AsyncStorage.getItem('discountUser').then((user) => {
        ...
        <View style={{ padding: 15, flex:1, }}>
            ...
            <View style={styles.button}>
               <View>
                    ...
                    <Text style={{fontWeight:'700',color:'#000'}}>Phone: {userData?.mobile}</Text>
                    </View>
                </View>             
            </View>
            ...
            <TouchableHighlight
             ...
             onPress={()=>{
                 AsyncStorage.removeItem('discountToken').then(()=>  dispatch(updateDiscountToken()))                                                 
                    AsyncStorage.removeItem('discountUser') .then(()=>navigation.dispatch(StackActions.replace('DiscountLogin'))
);
            }}>
             <View>
